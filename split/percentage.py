...
# get the length of train set and val set
...
# reorder the data index and target
...
# get the split by lengths on new data index and label
...
# a full process one label file content in one percentage
def default_process(seed, label_path, percentage, output_label_path_style):
    random.seed(seed)  # set random seed
    label_ori_default = load(label_path)  # load labels file
    label_idx_default, label_target_default = cat(label_ori_default)  # generate the data index and label
    label_train_len_default, label_val_len_default = percentage_length(len(label_idx_default), percentage)  # get
    # lengths of train and val
    label_idx_new_default_default, label_target_new_default = reorder(label_idx_default, label_target_default,
                                                                      len(label_idx_default))  # reorder the data index and label
    split_result_default = split(label_idx_new_default_default, label_target_new_default,
                                 [label_train_len_default, label_val_len_default])  # get split result of train and val
    split_result_default.append(label_ori_default[2])  # use origin test set
    file_name_default = os.path.basename(label_path)
    save(split_result_default, output_label_path_style, file_name_default, percentage)  # save the split


# a full process for all datasets on default setting
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()  # record the running setting
    ...
    parser.add_argument("--output_label_path_style", type=str,
                        ...
                        help="it is output label path style, you need to give {percentage}, {ori_name} in the style")
    parser.add_argument("--percentage", type=int, default=0.1, help="the percentage for training set")
    args = parser.parse_args()
    if not args.default:  # single file mode, same as function default_process
    ...
    else:  # directly use default setting for all datasets
